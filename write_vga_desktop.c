...
void showMemoryInfo(struct AddrRangeDesc* desc, char* varm, int page, int xsize, int color);
...
void CMain(void) {
    ...
    int mntCount = get_memory_block_count();
    char *pstr = intToHexStr(mntCount);
    io_sti();
    enable_mouse(&mdec);
    int data  = 0;
    int count = 0;
...
int xsize, int color) {
    ...
    showString(vram, xsize,x, y, color, pPageCnt);
    y+=16;
